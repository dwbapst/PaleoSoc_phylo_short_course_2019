...
![A tree of approaches that can be applied to trees](Summary_of_PCMS.png)
...
Note that this model uses two extra arguments: `correlation` which defines the expected structure in the residuals, in this case based on the phylogeny and a brownian motion model, and `weights` which is a modification to account for the different root to tip distances in our non-ultrametric tree - important for paleontologists.
