...
RevStudio is an analog of RStudio that allows one to edit, debug and run RevBayes scripts in an IDE environment.  However, the latest version does not seem to run on all computers.  Do not worry: *we will be using RevBayes in the terminal mode and not relying on RevStudio.*
