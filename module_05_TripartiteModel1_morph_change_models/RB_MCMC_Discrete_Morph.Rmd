---
title: "Discrete morphology - Cinctan Tree Inference"
...
>This directory should contain: [`Cinctans.nex`](data/Cinctans.nex).
...
First, we will create a joint on the branch lengths.
...
```
This prior specifies that branch lengths will be drawns from an exponential distribution with parameter .2. If you're not familiar with what an exponential distribution, try setting the below code to `eval = FALSE` to run the R code and visualize the distribution. Set `eval = FALSE` when you are done. 
...
We will add Gamma-distributed rate variation and specify moves on the parameter to the Gamma distribution.
...
This will create a .R file containing our analysis script. Move it into the `scripts` folder and rename it to mk_gamma.R
